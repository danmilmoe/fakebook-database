...
CREATE SEQUENCE cid
    START WITH 1
    INCREMENT BY 1;
CREATE TRIGGER make_city_id
    BEFORE INSERT ON Cities
    FOR EACH ROW
        BEGIN
            SELECT cid.NEXTVAL INTO :NEW.city_id FROM DUAL;
        END;
/
...
CREATE SEQUENCE pid
    START WITH 1
    INCREMENT BY 1;
CREATE TRIGGER make_program_id
    BEFORE INSERT ON Programs
    FOR EACH ROW
        BEGIN
            SELECT pid.NEXTVAL INTO :NEW.program_id FROM DUAL;
        END;
/
...
CREATE TABLE Albums (
    ...
    UNIQUE(album_id, album_owner_id),
...
ALTER TABLE Albums ADD CONSTRAINT cover_photo
FOREIGN KEY (cover_photo_id) REFERENCES Photos(photo_id) INITIALLY DEFERRED DEFERRABLE;

ALTER TABLE Photos ADD CONSTRAINT album
